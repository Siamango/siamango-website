...
import { AlertState, getAtaForMintV2, toDate } from "../../utils";
      ...
      if (props.candyMachineId) 
      {
        try 
        {
          ...
          if (cndy?.state.whitelistMintSettings) 
          {
            ...
            const mint = new PublicKey(cndy.state.whitelistMintSettings.mint);
            const token = (await getAtaForMintV2(props.connection, mint, anchorWallet.publicKey));
          ...
          {
            setDate(toDate(cndy.state.endSettings.number));
          ...
          if (e instanceof Error) {
            if (e.message === `Account does not exist ${props.candyMachineId}`) {
              setAlertState({
                open: true,
                message: `Couldn't fetch candy machine state from candy machine with address: ${props.candyMachineId}, using rpc: ${props.rpcHost}!`,
                severity: 'error',
                noHide: true,
              });
              ...
              setAlertState({
                open: true,
                message: `Coundn't reach ${props.rpcHost}`,//`Couldn't fetch candy machine state with rpc: ${props.rpcHost}! This probably means you have an issue with the REACT_APP_SOLANA_RPC_HOST value in your .env file, or you are not using a custom RPC!`,
                severity: 'error',
                noHide: true,
              });
            }
